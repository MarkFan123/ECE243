...
KEY_ISR:
	...
	stw     r15, 24(sp)
	stw     r20, 28(sp)
	movia	r3, 0xFF200050 /* Address of keys. */
	ldwio	r4, 0xC(r3)
	andi r15, r4, 0b1
	bne r15, r0, KEY0
	andi r15, r4, 0b10
...
KEY1:
	...
	ldwio r12, 0xc(r20)
	srli r11, r11, 1
	srli r12, r12, 1			   # multiply rate of toggle by 2
    stwio r11, 0x8(r20)         # write to the timer period register (low)
    stwio r12, 0xc(r20)         # write to the timer period register (high)
	movi r11, 0b0111            # enable continuous mode and start timer
	stwio r11, 0x4(r20)         # write to the timer control register to 
	br endkey
...
KEY2:
	...
	ldwio r12, 0xc(r20)
	slli r11, r11, 1
	slli r12, r12, 1			   # multiply rate of toggle by 2
    stwio r11, 0x8(r20)         # write to the timer period register (low)
    stwio r12, 0xc(r20)         # write to the timer period register (high)
	movi r11, 0b0111            # enable continuous mode and start timer
	stwio r11, 0x4(r20)         # write to the timer control register to 
